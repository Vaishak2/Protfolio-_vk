...
// var bike_book = document.getElementById('bike_book');
// var personal_book = document.getElementById('personal_book');

// function bikGall(){
//   var bike_book = document.getElementById('bike_book');
//   return
//   gallery.innerHTML = personal_book;
// }
// function perGall(){
//   gallery.innerHTML = personal_book;
// }
...
window.addEventListener('load', () => {
  ...
  if (galleryContainer) {
    let galleryIsotope = new Isotope(galleryContainer, {
      itemSelector: '.image-book',
      layoutMode: 'fitRows'
    });

    let galleryFilters = select('#gallery-filters_gal li', true);

    on('click', '#gallery-flters_gal li', function(e) {
      e.preventDefault();
      galleryFilters.forEach(function(el) {
        el.classList.remove('filter_gal-active');
      });
      this.classList.add('filter_gal-active');

      galleryIsotope.arrange({
        filter: this.getAttribute('data-filter_gal')
      });
    }, true);
  }
